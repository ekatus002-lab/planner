...
describe('AppShell mobile tab bar', () => {
  ...
  function calendarToolbar() {
    return screen.getByRole('toolbar', { name: 'Навигация по календарю' });
  }

  function habitsHeading() {
    return screen.getByRole('heading', { name: 'Привычки' });
  }
  ...
  it('shows only the Backlog (Задачи) section by default, marked as the current tab', () => {
    ...
    expect(isMobileHidden(calendarToolbar())).toBe(true);
    expect(isMobileHidden(habitsHeading())).toBe(true);
  });
  ...
  it('switches to the Calendar section when its tab is tapped, hiding the others', async () => {
    ...
    expect(isMobileHidden(calendarToolbar())).toBe(false);
  ...
  it('switches to the Habits section when its tab is tapped', async () => {
    ...
    expect(isMobileHidden(habitsHeading())).toBe(false);
    expect(isMobileHidden(screen.getByText('Backlog'))).toBe(true);
  });
  ...
  it('every section stays visible (not mobile-hidden) at the desktop breakpoint', () => {
    ...
    // `md:block` display override, so at `md` and above all three are shown
    // side-by-side regardless of which mobile tab is active.
    render(<AppShell userId="user-1" />);

    const calendarSection = calendarToolbar().closest('section');
    const habitsSection = habitsHeading().closest('section');
    expect(calendarSection?.className).toMatch(/\bhidden\b/);
    expect(calendarSection?.className).toMatch(/md:block/);
